...
async function getAtcoCodes(): Promise<RawAtcoString[]> {
    ...
    options.forEach((option) => {
        ...
        if (text) {
            ...
            logger.warn(
                `No text content found for option: "${JSON.stringify(option)}"`
            )
        }
    })
...
async function saveAtcoList(saveToJson: boolean = false): Promise<void> {
    ...
    if (saveToJson) {
        const jsonFilePath = path.join(
            ...
            "data/atco_codes.json"
        )
        if (fs.existsSync(jsonFilePath)) {
            logger.info(
                "ATCO codes JSON file already exists, skipping retrieval and save"
            )
            return
        }
    }
    ...
    for (const code of codeList) {
        try {
            const processedData: ProcessedAtcoCode =
                await processAtcoString(code) // process 1 code at a time
 ...
 */
async function processAtcoString(
    rawdata: RawAtcoString
): Promise<ProcessedAtcoCode> {
 ...
 * @see saveAtcoToDB
 */
async function saveAtcoToJson(
    processedCodes: ProcessedAtcoCode[]
): Promise<void> {
    ...
    logger.info(
        `Successfully saved ${processedCodes.length} ATCO codes to ${filePath}`
    )
}
...
async function saveAtcoToDB(
    ...
    logger.info(
        `Saved ${processedCodes.length} ATCO codes to the Atco collection`
    )
}
